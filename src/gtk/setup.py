...
class SetupDialog(Gtk.Dialog):
    ...
    def _save_settings(self, login_data: Dict[str, Any]) -> None:
        ...
        if 'oauth' in login_data:
            ...
            if self.advanced:
                ...
                shared_secret = config.get("login", "shared_secret").value
                identity_secret = config.get("login", "identity_secret").value
            new_configs = {
    ...
    def _do_login_callback(
        ...
        if future.result():
            ...
            self.next_button.connect("clicked", self._prepare_login_callback, next_stage)
    ...
    async def do_login(
        ...
        if self.relogin:
            shared_secret = config.get("login", "shared_secret").value
            identity_secret = config.get("login", "identity_secret").value
        elif self.advanced and (not shared_secret or not identity_secret):
